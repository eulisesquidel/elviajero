...
<script type="text/javascript">
      ...
      var url1 = url.substring(url.length-8,url.length) ;
      var url2="hotel.do"; 
      var url3="flota.do";
      var url4="uario.do"; 
      if (url1==url2) {
      	alert('No se puede eliminar hotel habiendo reservas pendientes');
	    window.location='hotel.do?reqCode=findData';
		} 
	  else {
		      if (url1==url3) {
		      	alert('No se puede eliminar flota habiendo reservas pendientes');
			    window.location='flota.do?reqCode=findData';
				} 
			  else {			
		      		if (url1==url4) {
		      			alert('No se puede eliminar usuario habiendo reservas pendientes');
			    		window.location='usuario.do?reqCode=findData';
					} 
			  		else {			
						alert('No se puede eliminar cliente habiendo reservas pendientes');
						window.location='cliente.do?reqCode=findData';
					} 
				} 
		} 
