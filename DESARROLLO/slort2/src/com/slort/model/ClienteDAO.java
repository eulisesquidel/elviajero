...
public class ClienteDAO extends HibernateDaoSupport {
	...
	public List<Cliente>  findByGUIPosibilities(Cliente objeto){

		DetachedCriteria criteria = DetachedCriteria.forClass(Cliente.class);
		
		if (BDBUtils.AdaptarCamposNulos(objeto.getNumDoc())!="")
			criteria.add(Restrictions.eq("numDoc", objeto.getNumDoc()));
		
		criteria.add(Restrictions.ilike("apellido", BDBUtils.AdaptarLike( BDBUtils.AdaptarCamposNulos(objeto.getApellido()))));
		criteria.add(Restrictions.ilike("nombre", BDBUtils.AdaptarLike( BDBUtils.AdaptarCamposNulos(objeto.getNombre()))));
		criteria.add(Restrictions.eq("fleliminado", new Byte("0")));
	...
	public List<Cliente>  findByGUIPosibilitiesReserva(String criterio){

		DetachedCriteria criteria = DetachedCriteria.forClass(Cliente.class);
		Criterion criteriodoc = Restrictions.ilike("numDoc", BDBUtils.AdaptarLike( BDBUtils.AdaptarCamposNulos(criterio)));	
		Criterion criterionom = Restrictions.ilike("nombre", BDBUtils.AdaptarLike( BDBUtils.AdaptarCamposNulos(criterio)));
		Criterion criterioape =	Restrictions.ilike("apellido", BDBUtils.AdaptarLike( BDBUtils.AdaptarCamposNulos(criterio)));
		Criterion criterionoeliminado =	Restrictions.eq("fleliminado", new Byte("0"));
		
		criteria.add(Restrictions.and(Restrictions.or(Restrictions.or(criteriodoc, criterionom) , criterioape), criterionoeliminado));
		
		//criteria.add(Restrictions.or(Restrictions.or(criteriodoc, criterionom) , criterioape));
		return  getHibernateTemplate().findByCriteria(criteria);
