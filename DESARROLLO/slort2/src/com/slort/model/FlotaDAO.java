...
public class FlotaDAO extends HibernateDaoSupport {
	...
	public List<Flota>  findByGUIPosibilities(Flota objeto){

		DetachedCriteria criteria = DetachedCriteria.forClass(Flota.class);
		
		if (BDBUtils.AdaptarCamposNulos(objeto.getLicencia())!="")
			criteria.add(Restrictions.eq("licencia", objeto.getLicencia()));
	...
	public List<Flota>  findByGUIPosibilitiesReserva(String criterio){

		DetachedCriteria criteria = DetachedCriteria.forClass(Flota.class);
		Criterion criteriolic = Restrictions.ilike("licencia", BDBUtils.AdaptarLike( BDBUtils.AdaptarCamposNulos(criterio)));	
		...
		criteria.add(Restrictions.or(Restrictions.or(criteriolic, criterionom) , criterioape));
		return  getHibernateTemplate().findByCriteria(criteria);
