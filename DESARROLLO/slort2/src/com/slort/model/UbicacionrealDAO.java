...
public class UbicacionrealDAO extends HibernateDaoSupport {
	...
	public List<Ubicacionreal> findByGoogleMapsView() {
		log.debug("finding Flota findByGoogleMapsView");
		try {
			String queryString = "from Ubicacionreal as ur where ur.gpsactivo=1 and ur.flota.activo=1";
			return getHibernateTemplate().find(queryString);
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
			throw re;
